...
<head>
    ...
    <title>Suscriptores - BilboSKP</title>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Suscriptores</h1>
                    </div>
                    
                        <!-- DataTables -->
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-primary">Listado de Suscriptores</h6>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                	<p>Los suscriptores que son responsables de un Centro educativo no se pueden dar de baja, hay que dar de baja el centro.</p>
                                	<table class="table table-bordered" id="dataTable" width="90%" cellspacing="0">
                                        ...
                                        <tbody>
                                        <c:forEach items="${suscriptores}" var="suscriptor">
                                        	<tr>
                                                <td>${suscriptor.nombre}</td>
                                                <td>${suscriptor.email}</td>
                                                <td>${suscriptor.usuario}</td>
                                                <td>${suscriptor.fecha_alta}</td>
                                                <td>${suscriptor.rol.nombre}</td>
                                                <td >
										            <form action="/bilboskp-asej/cancelarSuscriptor" method="post">
                                                		<input type="hidden" name="id_suscriptor" value="${suscriptor.id}">
                                                		<c:choose>
													        <c:when test="${suscriptor.rol.nombre == 'suscriptor'}">
														        <button type="submit" class="btn btn-danger  ">Dar de baja</button>
													        </c:when>
													        <c:when test="${suscriptor.rol.nombre == 'centro'}">
													           <button type="" class="btn btn-secondary" disabled>Dar de baja</button>
													        </c:when>
													    </c:choose>
