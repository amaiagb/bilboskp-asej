...
public class SuscriptorDAO {
	...
	public Suscriptor login(Suscriptor suscriptor) {
		...
		String sql = "SELECT id_suscriptor, nombre, usuario, contrasena, email, num_cupones, fecha_alta FROM suscriptor WHERE usuario=? AND contrasena=?;";		
		...
		try {
			...
			if(rs.next()) {
				...
				s.setNum_cupones(rs.getInt("num_cupones"));
